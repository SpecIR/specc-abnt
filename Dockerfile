...
#
# The base image already carries pandoc, the SpecCompiler engine, the external
# renderer binaries (deno, plantuml), and LibreOffice + Python UNO + the MS
# core fonts (docx field update / PDF export work out of the box). This image
# adds the abnt model tree under /opt/speccompiler/models/abnt/ — including
# the model-owned chart capability (CHART float + tools/echarts-render.ts) —
# and pre-warms the deno cache so charts render offline at runtime.
